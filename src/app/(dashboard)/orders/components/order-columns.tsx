...
export const OrderColumns: ColumnDef<TOrder>[] = [
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      return (
        <span className="font-medium text-gray-800">{invoiceId || "N/A"}</span>
      );
    },
  },
  {
    ...
    cell: ({ row }) => {
      ...
      return deliveryMan ? (
        <div className="flex items-center gap-x-3">
          ...
          <div>
            <h3 className="text-base font-semibold">{deliveryMan.name}</h3>
            <p className="text-sm text-gray-500">{deliveryMan.phone}</p>
          </div>
        </div>
      ) : (
    ...
    cell: ({ row }) => {
      ...
      return (
        <span className="text-sm text-gray-800">
          {deliveryLocation.streetAndNumber}
        </span>
      );
    },
  },
